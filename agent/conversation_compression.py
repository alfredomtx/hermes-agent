...
def _compression_model_label(agent: Any) -> str:
    ...
    failed_model = str(
        getattr(compressor, "_last_aux_model_failure_model", "") or ""
    ).strip()
    if failed_model:
        main_model = str(
            getattr(compressor, "model", "") or getattr(agent, "model", "") or ""
        ).strip()
        if main_model:
    ...
    remembered = str(
        getattr(agent, "_last_compression_model_label", "") or ""
    ).strip()
    if remembered:
        return remembered
    ...
    return (
        str(getattr(compressor, "model", "") or getattr(agent, "model", "") or "").strip()
        or "test-compressor"
    )
...
def _format_compaction_status_message(
    ...
    *,
    ...
    """Format one manual compaction lifecycle status message."""
...
def _emit_compaction_status(agent: Any, message: str, *, cli: bool = False) -> None:
    """Emit a manual compaction lifecycle status without breaking compression."""
    if cli:
        try:
            agent._vprint(
                f"{getattr(agent, 'log_prefix', '')}{message}",
                force=True,
            )
        except Exception:
            pass
...
class _CompactionStatusReporter:
    """Start, heartbeat, and terminal statuses for manual compression."""
    ...
    def __init__(
        ...
        *,
        ...
        self._interval_seconds = max(
            1.0,
            float(interval_seconds or _COMPACTION_HEARTBEAT_INTERVAL_SECONDS),
        )
    ...
    def start(self) -> "_CompactionStatusReporter":
        _emit_compaction_status(
            ...
            _format_compaction_status_message(
                "running", model_label=self._model_label
            ),
            cli=True,
        )
...
def compress_context(
    ...
    *,
    ...
    _compaction_status_reporter = None
    if force:
        # Manual /compress is deliberately visible even when the configured
        # context engine suppresses routine automatic maintenance statuses.
        _compaction_status_reporter = _CompactionStatusReporter(
            agent,
            _compression_model_label(agent),
        ).start()
        _compaction_status_emitted = True
    else:
    ...
    def _complete_compaction_lifecycle(state: str = "done") -> None:
        nonlocal _compaction_done_emitted
        if _compaction_done_emitted:
            return
        _compaction_done_emitted = True
        if _compaction_status_reporter is not None:
            if state == "done":
                _compaction_status_reporter.finish(state)
            else:
                # Keep the existing structured terminal edge for skipped and
                # aborted attempts; manual success is the path that carries
                # the model/elapsed lifecycle text.
    ...
    try:
        ...
        _complete_compaction_lifecycle("failed")
        _release_lock()
        _emit_compression_attempt_telemetry(
            agent,
            started_at=_attempt_started_at,
    ...
    try:
        ...
        if getattr(agent.context_compressor, "_last_compress_aborted", False):
            try:
                ...
                _complete_compaction_lifecycle("aborted")
                return messages, _existing_sp
            finally:
                _release_lock()
