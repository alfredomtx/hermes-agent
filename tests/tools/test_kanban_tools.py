...
def test_kanban_guidance_prompt_size_bounded(monkeypatch, tmp_path):
    """Sanity: the guidance block stays bounded (~4.5 KB) so it doesn't
    blow up the cached prompt."""
    monkeypatch.setenv("HERMES_KANBAN_TASK", "t_fake")
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
    ...
    # Upper bound raised from 4_096 -> 4_608: the hard completion gate now
    # documents the REQUIRED metadata.agent_receipt (its 9 fields + retry
    # semantics) inline in the lifecycle step, which legitimately pushed the
    # block just past 4 KB. Still tightly bounded so the cached prompt can't
    # balloon unnoticed.
